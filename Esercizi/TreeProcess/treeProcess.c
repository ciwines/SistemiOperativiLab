...
int processShouldTerminate = 0;
int processLevel = 0;
int masterProcesses[MAX_DEPTH] = {0};
int masterProcessesGroups[MAX_DEPTH] = {0};
...
// COn SIGUSR2 termina
void childProcessSignalHandler(int signo) {
    if (signo == SIGUSR1) {
        int pid = fork();

        // Child
        if (!pid) {
            printf("Setting Child gid at %d at level %d\n", masterProcessesGroups[processLevel + 1], processLevel);
            setpgid(0, masterProcessesGroups[processLevel + 1]);

            signal(SIGUSR1, childProcessSignalHandler);
            signal(SIGUSR2, childProcessSignalHandler);
            signal(SIGALRM, printSignalHandler);

            while(!processShouldTerminate);
...
int main() {
    ...
    // Geneo MAX_DEPTH master process
    ...
    for(int i = 0; i < MAX_DEPTH; i++) {
        ...
        if(!pid) {
            ...
            signal(SIGUSR1, masterProcessSignalHandler);
            signal(SIGUSR2, masterProcessSignalHandler);
        ...
        // Parent
            ...
            sleep(1);
            masterProcesses[i] = pid;
            masterProcessesGroups[i] = getpgid(pid);
        }
    }
    ...
    for(int i = 0; i < MAX_DEPTH; i++) {
        kill(-masterProcessesGroups[i], SIGUSR2);
    }
